...
def write_instance_to_example_files(instances, tokenizer, max_seq_length,
    ...
    input_ids_np = np.zeros((len(instances), max_seq_length), np.int32)
    segment_ids_np = np.zeros((len(instances), max_seq_length), np.int8)
    input_mask_np = np.zeros((len(instances), max_seq_length), np.int8)
    masked_lm_ids_np = np.ones((len(instances), max_predictions_per_seq), np.int32)
    masked_lm_weights_np = np.zeros((len(instances), max_seq_length), np.int8)
    masked_lm_positions_np = np.zeros((len(instances), max_predictions_per_seq), np.int32)
    next_sentence_labels_np = np.zeros((len(instances),), np.int8)
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        masked_lm_positions_np[inst_index] = masked_lm_positions
    ...
    # 切分保存hdf5数据
    f = h5py.File('data_training.hdf5', 'w')
    f.create_dataset('input_ids', data=input_ids_np, chunks=True, compression=4)
    f.create_dataset('input_mask', data=input_mask_np, chunks=True, compression=4)
    f.create_dataset('masked_lm_ids', data=masked_lm_ids_np, chunks=True, compression=4)
    f.create_dataset('segment_ids', data=segment_ids_np, chunks=True, compression=4)
    f.create_dataset('next_sentence_labels', data=next_sentence_labels_np, chunks=True, compression=4)
    f.create_dataset('masked_lm_positions', data=masked_lm_positions_np, chunks=True, compression=4)
    f.close()
...
def create_training_instances(input_files, tokenizer, max_seq_length,
    ...
    with open(input_files, encoding="utf-8", mode="r") as reader:
        while True:
